...
const App: React.FC = () => {
  ...
  const [interfacesStore] = React.useState(
    () => new InterfacesStore(launcherDataStore.clientLaunchSettingsStore)
  );
  const [mapsStore] = React.useState(() => new MapsStore());
  const [modsStore] = React.useState(() => new ModsStore());

  const [localGameStore] = React.useState(
    () =>
      new LocalGameStore(
      ...
      {launcherDataStore.isLoading || !launcherDataStore.gotData ? (
        ...
        <React.Fragment>
          ...
          >
            ...
            <TabPanel className="navigation-bar-content">
              <DemosPage demosStore={demosStore} uiState={uiStore.demosPage} />
            </TabPanel>

            <TabPanel className="navigation-bar-content">
              <SettingsPage
                clientLaunchSettingsStore={
                  launcherDataStore.clientLaunchSettingsStore
                }
