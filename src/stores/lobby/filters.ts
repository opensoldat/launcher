...
class ServerFiltersStore {
  ...
  filter(server: Server): boolean {
    ...
    if (this.players) {
      ...
      if (
        this.players === PlayersFilterOptions.NotEmpty &&
      ...
      if (
        this.players === PlayersFilterOptions.NotFull &&
      ...
      if (
        this.players === PlayersFilterOptions.NotEmptyAndNotFull &&
        (server.numPlayers === server.maxPlayers ||
          server.numPlayers + server.numBots === 0)
      ) {
        return false;
    ...
    if (this.gameMode) {
      if (
        (this.gameMode === GameModeFilterOptions.CaptureTheFlag &&
          server.gameMode !== GameModes.CaptureTheFlag) ||
        (this.gameMode === GameModeFilterOptions.DeathMatch &&
          server.gameMode !== GameModes.DeathMatch) ||
        (this.gameMode === GameModeFilterOptions.HoldTheFlag &&
          server.gameMode !== GameModes.HoldTheFlag) ||
        (this.gameMode === GameModeFilterOptions.Infiltration &&
          server.gameMode !== GameModes.Infiltration) ||
        (this.gameMode === GameModeFilterOptions.PointMatch &&
          server.gameMode !== GameModes.PointMatch) ||
        (this.gameMode === GameModeFilterOptions.RamboMatch &&
          server.gameMode !== GameModes.RamboMatch) ||
        (this.gameMode === GameModeFilterOptions.TeamDeathMatch &&
          server.gameMode !== GameModes.TeamDeathMatch)
      ) {
        return false;
      }
    }
    ...
    if (this.gameStyle) {
      if (
        (this.gameStyle === GameStyleFilterOptions.Advance &&
          !server.advance) ||
        (this.gameStyle === GameStyleFilterOptions.Realistic &&
          !server.realistic) ||
