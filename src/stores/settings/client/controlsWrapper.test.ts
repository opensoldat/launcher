...
test("Clears out all duplicate keys when setting new key", () => {
  ...
  const filterByNewKey = (binding: KeyBinding): boolean =>
    binding.key === newKey;

  const commonBindingsWithNewKey =
    controlsWrapper.controlsSettings.bindings.filter(filterByNewKey);
  const customBindingsWithNewKey =
    controlsWrapper.customBindings.bindings.filter(filterByNewKey);
