...
class InterfacesStore {
  ...
  @computed get gotInterfaces(): boolean {
    return (
      this.interfaceArchivesNames != null &&
  ...
  @computed get selectOptions(): SelectOption[] {
    ...
    if (this.clientLaunchSettingsStore.localMount) {
      interfacesNames = interfacesNames.concat(
        this.interfaceDirectoriesNames.slice()
      );
    } else {
    ...
    return ["Default"].concat(interfacesNames).map((interfaceName) => {
      return {
        label: interfaceName,
        value: interfaceName,
      };
